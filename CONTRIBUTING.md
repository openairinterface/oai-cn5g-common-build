...
1. Push your modified code to a new branch in the [GitHub repository](https://github.com/openairinterface/oai-cn5g-common-build).
   * Please use a short and descriptive branch name.

2. Create a pull request on [GitHub](https://github.com/openairinterface/oai-cn5g-common-build/pulls).
...
3. No CI runs on this repository's pull requests — these scripts and
   helpers are consumed as a submodule by the CN5G network-function
   repositories, so they are validated through an NF's pipeline, not here.
     * Point a network-function repo's `build/common-build` submodule at your
       branch and run that NF's pipeline to exercise your changes.
     * Summarise what you validated (which NF, which scenarios) in the pull
       request description.

4. A CI administrator will review your changes or assign them to a senior contributor for peer review.
   * The reviewer will check the code, commit messages, and the NF-pipeline.
   * All review discussions must be resolved before approval.

5. After approval, a CI administrator will merge the pull request.
   * The source branch will be deleted after the merge.

## Reporting Bugs

Please report software bugs or security issues through the [GitHub Issue Tracker](https://github.com/openairinterface/oai-cn5g-common-build/issues).
